...
export function assertSameSiteRequest(): void {
  ...
  // Some production proxies/browsers can label a same-origin server-function
  // fetch as `cross-site` after a redirect. Prefer the stronger explicit origin
  // metadata when it is present so legitimate requests from this deployment
  // are not blocked. This still rejects requests whose Origin/Referer belongs
  // to another site.
  ...
  if (referer) {
    try {
      ...
      // Ignore malformed Referer and continue with Fetch Metadata checks.
    }
  }
