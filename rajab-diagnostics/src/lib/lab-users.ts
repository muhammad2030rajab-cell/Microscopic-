...
/** See the identical helper + docstring in `platform-admin.ts`. */
...
export const createLabStaff = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }: {
    ...
    const { sql, lab_id, id: actorLabUserId } = await getOwnerContext(context.userId);
    ...
    try {
      ...
      // Same orphan-account bug as `createLab` in `platform-admin.ts`: without
      // this, a failed insert here (e.g. a username race lost against a
      // concurrent request) left a real, permanently-unlinked Better Auth
      // account behind — a login that could never reach any lab.
      await deleteAuthUser(sql, created.user.id);
      logEvent("error", "lab_staff.create.failed", { lab_id, error: String(error) });
      throw error;
    }

    await logAudit(sql, {
      labId: lab_id,
      actorUserId: actorLabUserId,
