...
export const createLabReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }: { context: { userId: string }; data: CreateLabReportInput }) => {
    ...
    if (data.patientId) {
      const existing = await sql<{ id: string; patient_code: string | null }>`
        select id, patient_code from patients where id = ${data.patientId} and lab_id = ${lab_id} limit 1
      `;
      if (!existing.length) throw new Error("PATIENT_NOT_FOUND");
      if (data.patientCode?.trim() && data.patientCode.trim() !== existing[0].patient_code) {
        throw new Error("PATIENT_CODE_MISMATCH");
      }
    }
    ...
    try {
      if (!data.patientId) {
        ...
        await sql.query(
          `update patients set full_name=$1, age=$2, gender=$3, phone=$4, national_id=$5, notes=$6, updated_at=current_timestamp where id=$7 and lab_id=$8`,
          [patientName, age, data.gender, data.phone?.trim() || null, data.nationalId?.trim() || null, data.notes?.trim() || null, data.patientId, lab_id],
        );
      }

      const resolvedPatientId = data.patientId || patientId;
      await sql.query(
        `insert into reports (id, lab_id, patient_id, sample_id, referring_doctor, status, notes, created_by)
         values ($1,$2,$3,$4,$5,'draft',$6,$7)`,
        [reportId, lab_id, resolvedPatientId, sampleId, data.doctor?.trim() || "Himself", data.notes?.trim() || null, lab_user_id],
      );
