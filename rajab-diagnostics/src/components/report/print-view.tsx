    ...
    >
      ...
      <div className="lab-titlebar">
        ...
        <div className="lab-report-status">
          <span className={cn("lab-status-dot", report.status === "approved" ? "lab-status-approved" : "lab-status-pending")} />
          <div>
            <p className="font-semibold">{report.status === "approved" ? "FINAL REPORT" : "PRELIMINARY REPORT"}</p>
            <p className="text-[0.62rem] text-muted">{report.status === "approved" ? "Approved / معتمد" : "Not yet approved / غير معتمد"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="lab-patient-grid mt-5">
        ...
        <div className="lab-info-cell lab-barcode-cell">
          <span className="lab-info-label">Sample Barcode / باركود العينة</span>
          <div className="lab-barcode-wrap">
            <span className="lab-barcode" aria-hidden="true" />
            <span className="font-mono text-[0.62rem] tabular-nums">{report.sampleId || visit}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center gap-3">
        ...
        <h2 className="font-display text-lg font-semibold tracking-[0.16em] sm:text-xl">LABORATORY RESULTS</h2>
        <span className="h-px flex-1 bg-ink" />
      </div>
      ...
      <div className="lab-final-grid mt-8">
        <div className="lab-flag-legend">
          <p className="font-semibold text-ink">Flag Meaning</p>
          <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
            <span>— Normal</span>
            <span className="inline-flex items-center gap-1 text-high"><ArrowUp className="size-3.5" /> High</span>
            <span className="inline-flex items-center gap-1 text-high"><ArrowDown className="size-3.5" /> Low</span>
            <span className="inline-flex items-center gap-1 text-high"><TriangleAlert className="size-3.5" /> Critical</span>
          </div>
        </div>
        <div className="lab-verification">
          <p className="text-[0.62rem] uppercase tracking-[0.12em] text-muted">Verified / اعتماد</p>
          <p className="mt-1 font-semibold">{report.status === "approved" ? "FINAL · APPROVED" : "PENDING APPROVAL"}</p>
          {report.status === "approved" && report.approvedAt ? (
            <p className="mt-0.5 text-[0.68rem] text-muted">Approved: {formatSlashDate(report.approvedAt)}</p>
          ) : null}
          <p className="mt-2 font-display text-base italic text-ink">{brand.doctorName || "Laboratory Director"}</p>
          <p className="text-xs font-semibold">Lab. Director</p>
          {brand.doctorDegree ? <p className="text-[0.68rem] text-muted">{brand.doctorDegree}</p> : null}
        </div>
      </div>
      ...
      <footer className="mt-6 border-t border-ink pt-3">
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-ink-soft">
          <span>{brand.website || brand.address || "Laboratory"}</span>
        ...
        <p className="mt-2 text-center text-[0.62rem] text-muted">
          This report is electronically generated. Please verify patient identity and sample information before clinical use.
        </p>
      </footer>
    </article>
  );
